...
func _process(delta: float) -> void:
	...
	if fill_direction == Vector2i.LEFT:
		var _scaled_value = float(value) / max * size.x
		%Fill.position = %Fill.position.lerp(Vector2(_scaled_value - size.x, 0), delta * 30.0)
		%Fill.size = %Fill.size.lerp(Vector2(_scaled_value, size.y), delta * 30.0)
